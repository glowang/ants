...
class Place(object):
    ...
    def remove_insect(self, insect):
        ...
        """
        if insect.is_ant == True:  
            if isinstance(insect,QueenAnt) and insect.impostor == True:
                if self.ant.container == False:
                    self.ant = None
                else:
                    self.ant.ant = None
                insect.place = None
...
class Ant(Insect):
    ...
    food_cost = 0
    blocks_path = True
    container = False #Added class attribute for #9
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        if random_or_none(self.place.bees) == None:
            next_location = self.place.entrance 
            while random_or_none(next_location.bees) == None or next_location == hive: 
                next_location = next_location.entrance 
                if next_location == None:
                    return None
            return random_or_none(next_location.bees) 
        return random_or_none(self.place.bees)  



        
                #return a random bee in the same place (closest, not in hive)
        # END Problem 3 and 4

    def throw_at(self, target): 
        #throw at the nearest bee in front of its place that's not in the Hive
        #in front of it: entrance(place) 
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        present_bees = self.place.bees 
        copy_of_present_bees = present_bees[:]
        #c = i for i in copy_of_present_bees      
        if self.armor <= 0:
            for c in copy_of_present_bees: 
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive): 
        ...
        while transitions < self.min_range:
            if current_location == None:
                return None 
            current_location = current_location.entrance 
            transitions += 1 
        next_location = current_location 
        while random_or_none(next_location.bees) == None:
            if next_location == None or next_location == hive:
                return None
            next_location = next_location.entrance 
        return random_or_none(next_location.bees)  
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive): 
        transitions = 0
        current_location = self.place 
        while random_or_none(current_location.bees) == None and transitions < self.max_range:
            transitions += 1  
            if current_location == None or current_location == hive:
                return None
            current_location = current_location.entrance 
...
class NinjaAnt(Ant):
    ...
    # BEGIN Problem 7
    food_cost = 5 
    blocks_path = False #NinjaAnt still has this attribute; it just turns it off
    implemented = True   # Change to True to view in the GUI
    # END Problem 7

    def action(self, colony):
        # BEGIN Problem 7  
        available_bees = self.place.bees
        copy_of_available_bees = available_bees[:]
        for individual_bee in copy_of_available_bees:
            individual_bee.reduce_armor(self.damage)
...
class HungryAnt(Ant):
    ...
    def action(self, colony): 
        ...
        self.digesting = self.time_to_digest - self.round_counter 


               


        # if disguesting, decrement its digesting counter
        # else eat a random bee AND restart the disgesting timer again 
        # END Problem 6 
...
class BodyguardAnt(Ant):
    ...
    implemented = True   # Change to True to view in the GUI
    container = True
    damage = 0 #added here to match with tankant  
    # END Problem 9
    ...
    def contain_ant(self, ant):
        ...
        self.ant = ant 
        '''
        self.is_ant = ant.is_ant
        self.implemented = ant.implemented  # Only implemented Ant classes should be instantiated
        self.food_cost = ant.food_cost
        self.blocks_path = ant.blocks_path
        self.container = ant.container  
        '''
        #self.armor = ant.armor
        '''if self.armor <= 0:
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.ant:
            self.ant.action(colony)
        for b in copy_of_available_bees:
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        #nonlocal num_of_initiations 
        self.armor = 1
        if QueenAnt.num_of_initiations == 0:
            self.impostor = False 
            QueenAnt.num_of_initiations += 1 
            self.doubled = []
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        # ThrowerAnt.self.throw_at(self.nearest_bee(colony.hive)) #this basically repeats the exact action of the grandparent class 
        #both real and impostor queen can do this 
        #Can I refer to its grandparent superclass by using ThrowerAnt directly? 
        if self.impostor == False: 
            #action 2. double the damages of all bees behind her 
            #all the places behind her:
            ScubaThrower.action(self,colony) 
            locations = [] 
            current_location = self.place 
            while current_location.exit != None:
                locations.append(current_location.exit) 
                current_location = current_location.exit 
            for i in locations: 
                if i.ant != None:
                    if i.ant not in self.doubled:
                        i.ant.damage *= 2
                        self.doubled.append(i.ant)
                    if i.ant.container == True and i.ant.ant != None and i.ant.ant not in self.doubled:
                        i.ant.ant.damage *= 2
                        self.doubled.append(i.ant.ant)
        else:
            #I want reduce armor to 0 upon FIRST action 
